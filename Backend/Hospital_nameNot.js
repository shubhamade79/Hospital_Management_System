...
app.post('/getAadhaarDetails', async (req, res) => {
    ...
    try {
        ...
        res.json({ full_name: aadhaar.full_name });
    } catch (error) {
        ...
        if (user.password === password) {
            return res.json({
                ...
                user: {
                    ...
                    hdmis_number: user.hdmis_number,
...
app.post("/api/patient_detail_for_appointment", async (req, res) => {
    try {
        const { hdmis_number, age, gender, contact_number, email, address } = req.body;
...
app.post("/api/book_appointment", async (req, res) => {
    try {
        const { hdmis_number, doctor_id, date, time } = req.body;

        if (!hdmis_number || !doctor_id || !date || !time) {
            return res.status(400).json({ error: "All fields are required" });
        }

        // Create a new appointment
        const appointment = new Appointment({ hdmis_number, doctor_id, date, time });
        await appointment.save();

        // Get the generated appointment_id
        const { appointment_id } = appointment;

        // Update Patient record with appointment_id
        await Patient.updateOne(
            { hdmis_number },
            { $set: { appointment_id } }
        );

        // Update Doctor record with appointment_id
        await Doctor.updateOne(
            { doctor_id },
            { $set: { appointment_id } }
        );

        res.json({ message: "Appointment booked successfully", appointment_id });
...
const MedicalRecordSchema = new mongoose.Schema({
    ...
    doctor_id: { type: String, required: true, index: true }, // Store as string (same as MySQL)
...
app.post("/updatePatientRecord", async (req, res) => {
    try {
        const { hdmis_number, medicine, disease } = req.body;

        if (!hdmis_number || !medicine || !disease) {
            return res.status(400).json({ message: "Missing required fields" });
        }

        // Fetch a random doctor
        const randomDoctor = await Doctor.aggregate([{ $sample: { size: 1 } }]);
        if (randomDoctor.length === 0) {
        ...
        const newRecord = new MedicalRecord({
            hdmis_number,
            disease,
            doctor_id: randomDoctor[0]._id,  // Assign random doctor ID
            medicine
        });
...
app.get('/doctors', async (req, res) => {
  ...
  app.post("/add-doctor", async (req, res) => {
    try {
      const { doctor_id, password, phone, name, specialization, email, hospital_name, admin_id, start_time, end_time } = req.body;
...
app.post("/add-receptionist", async (req, res) => {
  try {
    const { user_id, password, contact, name, hospital_name, admin_id } = req.body;

    if (!user_id || !password || !contact || !name || !hospital_name || !admin_id) {
      return res.status(400).json({ success: false, message: "All fields are required" });
    }
...
app.get("/get-hospital-name", async (req, res) => {
    ...
    try {
        ...
        if (hospitalAdmin) {
            res.status(200).json({ hospital_name: hospitalAdmin.hospital_name });
        } else {
